...
sim_df <- do_sims(n_sims, sym_3sp_dr, 8, b2_change_fun = b2_change_cont, phases = n_phases, 
...
# Greater number of generations per cycle
# =========================================================
# =========================================================
...
b1s <- c(0.25, -0.25)
r_sd <- 0.2
b2_change_cont <- function(rel_t, phases_done, lambda = 0.2) {
    beta1 <- abs((2 * rel_t) - 1)
    b3_2 <- (1 - beta1) * lambda
    b2_3<- beta1 * lambda
    return(c(b3_2, b2_3))
...
sim_df <- do_sims(n_sims, sym_3sp_dr, 7, b2_change_fun = b2_change_cont, phases = n_phases, 
                  b1s = b1s, phase_len = phase_len, r_sd = r_sd) %>%
    gather(species, abundance, -generation, -sim, factor_key = TRUE)
